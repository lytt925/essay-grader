...
class Interface(tk.Tk):
    ...
    def setup_left_frame(self):
        ...
        self.label_filepath.grid(row=0, column=0, sticky="ew", padx=(2, 2), pady=5)
        self.entry_filepath = tk.Entry(self.left_frame)
        self.entry_filepath.grid(row=0, column=1, sticky="ew", padx=(2, 2), pady=5)
        self.button_browse = tk.Button(self.left_frame, text="Browse", command=self.browse_dir)
        self.button_browse.grid(row=0, column=2, sticky="ew", padx=(2, 15), pady=5)
    ...
    def setup_right_frame(self):
        ...
        self.name_dropdown = ttk.Combobox(self.right_frame, values=[], width=35, state="readonly")
        self.name_dropdown.grid(row=0, column=1, padx=(2, 10), pady=5, sticky="w")
        self.name_dropdown.bind("<<ComboboxSelected>>", self.on_essay_select)

        self.button_next = tk.Button(self.right_frame, text="Next", command=self.next_essay)
        self.button_next.grid(row=0, column=2, padx=(2, 10), pady=5, sticky="w")

        # Email Subject
        self.label_emailsubject = tk.Label(self.right_frame, text="Email Subject")
        self.label_emailsubject.grid(row=1, column=0, sticky="w", padx=(10, 2), pady=5)
        self.entry_emailsubject = tk.Entry(self.right_frame)
        self.entry_emailsubject.grid(row=1, column=1, sticky="ew", padx=(2, 2), pady=5)
